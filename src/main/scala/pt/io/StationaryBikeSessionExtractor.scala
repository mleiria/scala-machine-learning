...
/**
 * Extractor for stationary bike sessions from Samsung Health data.
 */
object StationaryBikeSessionExtractor {

  private val STATIONARY_BIKE_EXERCISE_TYPE = 15003

  /**
    * Extracts all stationary bike sessions from the provided Samsung Health data directory.
    *
    ...
    * @return A Dataset of Exercise objects filtered for stationary bike sessions.
    */
  def extractStationaryBikeSessions(spark: SparkSession, dataDir: String): Dataset[Exercise] = {
    ...
    // 5. Filter for stationary bike sessions (standard Samsung Health type for stationary cycling is 15003)
    exercises.filter(_.exercise_type.contains(STATIONARY_BIKE_EXERCISE_TYPE))
  }
}
