...
object LSTM {
  ...
  def forward(x: Seq[DenseVector[Double]],
    ...
    for (xt <- x) {
      ...
      // Cell State
      val c = ((f * cPrev) + (i * cTilde)).asInstanceOf[DenseVector[Double]]
      ...
      // Hidden State
      val h = (o * NormalizeUtils.tanh(c)).asInstanceOf[DenseVector[Double]]
  ...
  def backward(x: Seq[DenseVector[Double]],
    ...
    val dWf = DenseMatrix.zeros[Double](hiddenDim, hiddenDim + inputDim)
    val dbf = DenseVector.zeros[Double](hiddenDim)
    val dWi = DenseMatrix.zeros[Double](hiddenDim, hiddenDim + inputDim)
    val dbi = DenseVector.zeros[Double](hiddenDim)
    val dWc = DenseMatrix.zeros[Double](hiddenDim, hiddenDim + inputDim)
    val dbc = DenseVector.zeros[Double](hiddenDim)
    val dWo = DenseMatrix.zeros[Double](hiddenDim, hiddenDim + inputDim)
    val dbo = DenseVector.zeros[Double](hiddenDim)
    ...
    for (t <- (tMax - 1) to 0 by -1) {
      ...
      val dOutput = (dh * NormalizeUtils.tanh(state.c) * (state.o * (-state.o + 1.0))).asInstanceOf[DenseVector[Double]]
      val dWo_t = dOutput * vt.t
      dbo += dOutput
      dWo += dWo_t

      // Cell state gradients
      val dC = ((dh * state.o * (-pow(NormalizeUtils.tanh(state.c), 2) + 1.0)) + dCNext).asInstanceOf[DenseVector[Double]]

      // Candidate state gradients
      val dcTilde = (dC * state.i * (-pow(state.cTilde, 2) + 1.0)).asInstanceOf[DenseVector[Double]]
      val dWc_t = dcTilde * vt.t
      dbc += dcTilde
      dWc += dWc_t

      // Input gate gradients
      val di = (dC * state.cTilde * (state.i * (-state.i + 1.0))).asInstanceOf[DenseVector[Double]]
      val dWi_t = di * vt.t
      dbi += di
      dWi += dWi_t
      ...
      val df = (dC * hPrevC * (state.f * (-state.f + 1.0))).asInstanceOf[DenseVector[Double]]
      val dWf_t = df * vt.t
      dbf += df
      dWf += dWf_t
      ...
      dhNext = dV(0 until hiddenDim).asInstanceOf[DenseVector[Double]]
      dCNext = (state.f * dC).asInstanceOf[DenseVector[Double]]
    }
